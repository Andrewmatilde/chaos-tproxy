...
pub struct TcpSocketOptions {
    ...
    /// Set IP_TRANSPARENT on the listening socket so it can accept
    /// connections whose destination IP is not local to this host
    /// (redirected here via TPROXY / bpf_sk_assign / iptables REDIRECT).
...
fn apply_tcp_socket_options(sock: &TcpSocket, opt: Option<&TcpSocketOptions>) -> Result<()> {
    ...
    #[cfg(unix)]
    if let Some(true) = opt.ip_transparent {
